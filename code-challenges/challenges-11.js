...
function fullName(arr) {
    ...
    let arr1;
     ...
     arr1= arr.map(arr => arr.firstName + " " + arr.lastName);
...
function gradesAvg(arr) {
    ...
    arr1= arr.map(arr => {
        var sum= arr.gradsList.reduce((acc,grade) => acc+grade,0);
        var avg=sum/arr.gradsList.length;
     return {
        ... arr,
        avg: avg    };
    } );
...
function studentsResult(arr) {
    ...
    arr1= arr.map(arr => (sum+=arr.gradsList[i])/gradsList.length);
}
return arr1;
